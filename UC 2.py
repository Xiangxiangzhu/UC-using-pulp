...
amount=3
#%%
TimeCostGurobi=np.zeros(amount)
TimeCostCplex=np.zeros(amount)
TimeCostGurobi_2=np.zeros(amount)
TimeCostCplex_2=np.zeros(amount)

#%%
for UnitNumber in range(1,2*amount+1):
    if UnitNumber>amount:
        unit_amount=UnitNumber-amount
    else:
        unit_amount=UnitNumber-0
    print(unit_amount)
#    unit_amount=UnitNumber#机组数目*10
    #%% define function
    ...
    NL=4#出力近似分段为4段
    ND=15#开机费用近似为15段
    result_var_v=np.ones((10*unit_amount,24))#结果显示
    result_var_p=np.ones((10*unit_amount,24))#结果显示
    ...
    system_data_1_0=np.array([[455,150,8,8,8],
                           ...
                           [55,10,1,1,-1],
                           [55,10,1,1,-1]])
    system_data_1=np.tile(system_data_1_0,(unit_amount,1))
    ...
    system_data_2=np.tile(system_data_2_0,(unit_amount,1))
    #load_demand
    load_demand=np.array([700,750,850,950,1000,1100,1150,1200,1300,1400,1450,1500,
                         1400,1300,1200,1050,1000,1100,1200,1400,1300,1100,900
                         ,800])[np.newaxis,:]
    load_demand=unit_amount*load_demand
    ...
    #%% 目标函数
    UC += (pulp.lpSum( (pulp.lpSum(  (var_cp[j][k] ) for j in range(row_matrix_cu)   ))  for k in range(col_matrix_cu)))
    ...
    '''
    #%% 求解
    solve_1=pulp.GUROBI_CMD(options=[("MIPgap", 0.0001)])
    solve_2=pulp.CPLEX()
    if UnitNumber>amount:
        solve=solve_2
    else:
    ...
    #UC.solve(pulp.CPLEX())
    UC.solve(solve)
    #UC.solve(pulp.GUROBI_CMD(options=[("MIPgap", 0.0001)]))
    time_end=time.time()
    if UnitNumber>amount:
        TimeCostCplex_2[unit_amount-1]=time_end-time_start
        TimeCostCplex[unit_amount-1]=time_end_0-time_start_0
    else:
        TimeCostGurobi_2[unit_amount-1]=time_end-time_start
        TimeCostGurobi[unit_amount-1]=time_end_0-time_start_0
...
plt.plot(range(1,amount+1),TimeCostCplex)
plt.plot(range(1,amount+1),TimeCostGurobi)    
#    print("Minimun Cost =",pulp.value(UC.objective))
#    print('totally cost',time_end-time_start)
# =============================================================================
# 
# for i in range(10*unit_amount):
#     for j in range(24):
#         result_var_v[i,j]=pulp.value(var_v[i][j])
# print(result_var_v)
# 
# for i in range(10*unit_amount):
#     for j in range(24):
#         result_var_p[i,j]=pulp.value(var_p[i][j])
# print(result_var_p)
# 
# 
# =============================================================================
...
plt.plot(range(1,amount+1,1),TimeCostCplex,'-',label="Cplex")
plt.plot(range(1,amount+1,1),TimeCostGurobi,'-',label="Gurobi")    
plt.legend(loc='upper left')
plt.xlabel('Unit amount')
plt.ylabel('Solving Time, Second')
plt.title('UC Problem Solving Time Using Different Solver')
plt.legend()
plt.grid()

#%%

plt.plot(range(1,amount+1,1),TimeCostCplex_2/TimeCostCplex,'-',label="Cplex")
plt.plot(range(1,amount+1,1),TimeCostGurobi_2/TimeCostGurobi,'-',label="Gurobi")    
plt.legend(loc='upper left')
plt.xlabel('Unit amount *10')
